...
class ShoppingCartPage extends StatefulWidget {
  ...
  @override
  State<StatefulWidget> createState() => ShoppingCartPageState();
}

class ShoppingCartPageState extends State<ShoppingCartPage> {
  List<VendorModel> shippings = [];
  double latitude;
  double longitude;
  var currentLocation;

  loadNearestShipping() async {
    await getCurrentLocation();
    shippings = await HomeApi.getNearbyShippingList(
  ...
  void setCustomMapPin() async {
    pinLocationIcon = await BitmapDescriptor.fromAssetImage(
        ImageConfiguration(size: Size(12, 12)), 'assets/shiii.jpeg');
              ...
              children: [
                ...
                Container(
                  ...
                  child: TextButton(
                    ...
                    onPressed: () async {
                      try{
                        var result = await HomeApi.addOrder(ShippigId: winch.userId);
                        if(result){
                            // Fluttertoast.showToast(msg: "تم إضافة الطلب بنجااااااااح بإنتظار اتصال المندوب",toastLength: Toast.LENGTH_LONG);
                    ...
                    child: Text('choose'.tr(),style: TextStyle(
                      color: Colors.white
                    ),),
                  ),
                )
